...
defineTypeNameAndDebug(BeetstraDragPost, 0);
...
BeetstraDragPost::BeetstraDragPost
(
...
void BeetstraDragPost::setForce
(
    ...
    {	   
	  ...
	  nuf = nufField[cellI];
		...
		CD = 24./Rep*(1.+0.15*pow(Rep,0.687))*pow(voidfraction,-1.65);
	      ...
	      drag = (10.0*alps/(voidfraction*voidfraction) + voidfraction*voidfraction*(1.0+1.5*pow(alps,0.5)) + 0.413*Rep/(24.0*voidfraction*voidfraction)*(1.0/voidfraction+3.0*voidfraction*alps+8.4*pow(Rep,-0.343))/(1.0+pow(10.0,3.0*alps)*pow(Rep,-0.5-2.0*alps))) * 3.0*3.14159265358979*nuf*rho*ds*Ur*voidfraction;
	  }

          if(verbose_ && index <1 )
          {
	      Pout << tab << "Beetstra Drag"  << endl;
...
void BeetstraDragPost::setForceEulerian
(
	const volScalarField& voidfraction_,
	const volVectorField& U_,
	cfdemCloud& particleCloud_,
...
) const
{
    ...
    {	   
      ...
      if(verbose_ && cellI <1 )
      {
	  Pout << tab << "Beetstra Drag-scalarField radii"  << endl;
	  Pout << tab << "index = " << cellI << endl;
          Pout << tab << "Up = " << Us_[cellI] << endl;
          Pout << tab << "Ur = " << Ur << endl;
          Pout << tab << "dp = " << ds << endl;
    ...
    {	   
	  ...
	  mp = rhoParticle_*Volp;
		...
		CD = 24./Rep*(1.+0.15*pow(Rep,0.687))*pow(voidfraction,-1.65);
          ...
          if(verbose_ && index <1 )
          {
	      Pout << tab << "Beetstra Drag-Lists"  << endl;
	      Pout << tab << "index = " << index << endl;
...
) const
{
    ...
    {	       
	  ...
	  // calc model coefficient F0
          scalar F0 = 10.f * alps / voidfraction / voidfraction
                     + voidfraction * voidfraction * ( 1.0+1.5*sqrt(alps) );

          // calc model coefficient G0
          scalar G0 =  0.01720833 * Rep / voidfraction / voidfraction  //0.0172083 = 0.413/24
                      *  ( 1.0 / voidfraction + 3.0 * alps  * voidfraction + 8.4 
                                                  * powf(Rep,-0.343) ) 
                    /  ( 1.0 + powf( 10., 3.0* alps ) 
                             * powf( Rep,-(1.0+4.0*alps)/2.0 ) );

          // calc model coefficient beta
          drag =  18.*nuf*rho/(ds*ds) 
                    * alps * voidfraction
                    *(F0 + G0) * Ur ;	 	 
         /* */
      }
      ...
      if(verbose_ && cellI <1 )
      {
	  Pout << tab << "Beetstra Drag-ListRadii"  << endl;
	  Pout << tab << "index = " << cellI << endl;
          Pout << tab << "Up = " << Us_[cellI] << endl;
          Pout << tab << "Ur = " << Ur << endl;
          Pout << tab << "dp = " << ds << endl;
...
void BeetstraDragPost::setForceParcel
(
    const volScalarField& voidfraction_,
    const volVectorField& U_,
