	...
	forAll(timeDirs, timeI)
	{
		...
		if(fluidCoarsening)
		{
		  forAll(filterWidth,fWidth)
		  {
		    ...
		    {
			    // Particle coarsening.H
			    #include "particleCoarsening.H"
			    	    
			    //- Conditionally averaging		    
			    if(conditionalAveraging)
			    {
		    		#include "conditionalAveLag.H"  
	  		    }	
	  		    //- Multi-variable averaging
			    if(multConditionalAveraging) 
			    {
		    		#include "multiVarsConditionalAveLag.H"
			    }
			    
			    if(parcelCoarsening)
		    	    {
		        	#include "parcelCoarsening.H"		      
		            }			    
		    }
		    
		    //-Eulerian filtering
		    #include "eulerianVars.H"				    
		    #include "eulerianFiltering.H" 			    		    	

		    //- Conditionally averaging		    
		    if(conditionalAveraging)
		    {
		    	#include "conditionalAveEul.H"  
	  	    }	
	  	    //- Multi-variable averaging
		    if(multConditionalAveraging) 
		    {
		    	#include "multiVarsConditionalAveEul.H"
		    }
	  	    //- Three-variable averaging
		    if(threeConditionalAveraging) 
		    {
		    	#include "threeVarsConditionalAveEul.H"
		    }		    		    
